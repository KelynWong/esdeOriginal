...
if ($searchDesignFormContainer.length != 0) {
        ...
        axios({
            ...
            .then(function(response) {
                ...
                for (let index = 0; index < records.length; index++) {
                    ...
                    $cardBody.append($('<h5></h5>').addClass('card-title').html(record.fullname));
                    ...
                    $cardBody.append($('<p></p>').addClass('card-text').html(record.email));
    ...
    function clickHandlerForPageButton(event) {
        ...
        const baseUrl = 'http://localhost:5000';
        let userId = localStorage.getItem('user_id');
        let pageNumber = $(event.target).html().trim();
        let searchInput = $('#searchInput').val();
        ...
        axios({
            ...
            .then(function(response) {
                ...
                for (let count = 1; count <= totalPages; count++) {
                    ...
                    $pageButtonContainer.append($('<button class="btn btn-primary btn-sm"></button>').html(count).on('click', clickHandlerForPageButton));
                }
            })
            .catch(function(response) {
                //Handle error
